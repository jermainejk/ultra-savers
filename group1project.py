...
def update(id):
    ...
    for ulist in user_file:
        ...
        if list[0] == userid and list[2] == oldsetter and list[3] == oldgoal:
            usergoal = Goal.Goal(list[0], list[1], list[2], list[3], list[4], list[5],list[6])
    ...
    if request.method == 'POST' and form.validate():
        if form.type.data =="parent":
            userlist3 = []
            #userlist3 = AddGoalProcess.EditUser(goal.get_setter(), goal.get_goal(), goal.get_amount(), goal.get_duration())
            ...
            AddGoalProcess.EditUserGoal(userid, oldsetter, oldgoal, setter, goal, amount, duration)
  ...
  #          userlist3 = AddGoalProcess.EditUser(goal.get_setter(), goal.get_goal(), goal.get_amount(),
                                                #goal.get_duration())
            userid = form.userid.data
            type = form.type.data
            setter = form.setter.data
...
def AddGoal():
    ...
    if request.method == 'POST' and form.validate():
        if form.type.data =="parent":
            ...
            AddGoalProcess.registerNewUser(userid,type,setter,goal,amount,duration,str(startdate))
    ...
    return render_template('AddGoals.html', form=form,users = userslist,count=len(userslist),usersid = useridlist)
